...
# ============================================================
# EXPLICIT SETTINGS: NO SEARCHING, NO MANIFEST
# ============================================================

BASE = Path("/home/idies/workspace/Storage/elenceq/mhd_work/jhtdb_mhd1024")

U_FILE = BASE / "raw/mhd1024_velocity_t0057_x289-736_y289-736_z289-736.npy"
B_FILE = BASE / "raw/mhd1024_magneticfield_t0057_x289-736_y289-736_z289-736.npy"

OUTDIR = BASE / "processed/perpBL_v1/smoke"
OUTDIR.mkdir(parents=True, exist_ok=True)

OUT_NPZ = OUTDIR / "perpBL_smoke_448_v0_results.npz"
OUT_JSON = OUTDIR / "perpBL_smoke_448_v0_summary.json"

R_LIST = np.array([32, 64, 128, 192], dtype=np.float64)

N_SAMPLE = 30000
NPHI = 8
SEED = 12345

# For fast smoke test only: B_L is computed on block-averaged magnetic field.
BL_DOWNSAMPLE = 4

# Chunk size for interpolation.
CHUNK = 200000
...
def ensure_vec_last(arr):
    """Return vector field as shape (nx, ny, nz, 3)."""
    if arr.ndim != 4:
...
def unit_rows(v):
    """Normalize rows of an (N,3) array."""
...
def trilinear_vec(field, coords):
    """Trilinear interpolation of vector field at coords."""
...
def compute_jmag(B):
    """Compute |curl B| in grid-index units. Good enough for ranking top |j|."""
...
def perp_basis(bhat):
    """Construct e1,e2 perpendicular to bhat."""
    n = bhat.shape[0]
...
print("=" * 80, flush=True)
print("perpBL smoke test: 448^3 cube", flush=True)
print("=" * 80, flush=True)
...
print("\nSaving smoke outputs ...", flush=True)
...
with open(OUT_JSON, "w") as f:
    json.dump({
        "description": "perpBL smoke test for 448^3 cube",
